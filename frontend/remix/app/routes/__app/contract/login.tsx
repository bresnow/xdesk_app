...
export let action: ActionFunction = async ({ params, request, context }) => {
  ...
  let auth = new Auth({ request, context });
  let { alias, password } = await FormEntry<{alias:string; password:string}>(request);
  ...
  return redirect(request.headers.get('Referer') ?? '/contract')
};
