...
export function colorFrom256(number: number) {
...
export function colorFromRGB(R: string | number | any[], G: number, B: number, A: any) {
...
export function colorFromHex(R: string | any[], G: any, B: any, A: number) {
    if ((Array.isArray(R)))
        R = R.join('');
...
export function colorFromText(text: any) {
...
export function fontFromText(text: any) {
...
export function _getIsImage(args: string | IArguments | any[]) {
    if (args.length !== 1)
        return false;
...
export function _getIsTransparent(A: number) {
    return (typeof A !== 'undefined' && A <= maxTransparency);
}

export function _stringToDec(str: string) {
    str = str || '';
 ...
 export class Printer {
    ...
    _getPaintedArgs(args: IArguments) {
        let str = '';
    ...
    _getPaintedString(text: string, noReset: boolean) {
        let str = TERM_ESC;
    ...
    _printImage(pixelsArr: any, output: string) {
        let total = '';
        ...
        for (let row of pixelsArr) {
            ...
            for (let i = 0; i < row.length; i++) {
                ...
                if (nextPixel && pixel.every((value: any, index: string | number) =>
