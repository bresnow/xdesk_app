...
export const DragSourceIconView = GObject.registerClass(
  {
    GTypeName: "DragSourceIconView",
  },
  class extends Gtk.IconView {
    _init() {
      super._init();
      ...
      this.enable_model_drag_source(
        Gdk.ModifierType.BUTTON1_MASK | Gdk.ModifierType.MOD1_MASK,
        null,
        Gdk.DragAction.DEFAULT | DRAG_ACTION
      );
      this.connect(
        "drag-data-get",
        (widget, drag_context, data, info, time) => {
          ...
          if (info === TARGET_ENTRY_TEXT) {
            ...
            let pixbuf = this.get_model().get_value(
              selected_iter,
              COLUMN_PIXBUF
            );
...
export const DropArea = GObject.registerClass(
  ...
  class extends Gtk.Label {
    _init() {
      ...
      this.connect(
        "drag-data-received",
        (widget, drag_context, x, y, data, info, time) => {
          if (info === TARGET_ENTRY_TEXT) {
