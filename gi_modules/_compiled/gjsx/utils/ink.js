...
export function colorFrom256(number) {
  if (typeof number === "undefined")
    number = Math.floor(Math.random() * 256) + 1;
  return `38;5;${number || 0}`;
}
export function colorFromRGB(R, G, B, A) {
  ...
  if (_getIsTransparent(A)) return Color.DEFAULT;
  R = R || 0;
  G = G || 0;
  B = B || 0;
  return `38;2;${R};${G};${B}`;
}
export function colorFromHex(R, G, B, A) {
  ...
  while (str.length < alphaIndex) str += "0";
  A =
    str.length > alphaIndex
...
export function fontFromText(text) {
  let arr = Object.keys(Font);
  let value = _stringToDec(text);
  return Font[arr[(value % (arr.length - 1)) + 1]];
}
export function _getIsImage(args) {
  if (args.length !== 1) return false;
  let arg = args[0];
  let argType = typeof arg;
  if (argType === "string" || argType === "number") return false;
...
export function _getIsTransparent(A) {
  return typeof A !== "undefined" && A <= maxTransparency;
}
export function _stringToDec(str) {
  str = str || "";
  let len = str.length;
  let total = 0;
  while (len--) total += Number(str.charCodeAt(len).toString(10));
  return total;
}
export class Printer {
  ...
  constructor(opts) {
    ...
    for (let def in defaults) {
      this[def] = typeof opts[def] !== "undefined" ? opts[def] : defaults[def];
    }
  }
  print() {
  ...
  set background(value) {
    ...
    if (valueType === "string") {
      value = value[2] === ";" ? "4" + value.substring(1) : Number(value);
    }
    this._background =
      valueType === "object"
        ? null
  ...
  _getPaintedArgs(args) {
    ...
    for (let arg of args) {
      ...
      let painted = this._getPaintedString(arg, false);
  ...
  _getPaintedString(text, noReset) {
    ...
    for (let option of ["font", "color", "_background"]) {
      ...
      str +=
        optionType === "number" || optionType === "string"
          ? this[option]
    ...
    str += text;
    return noReset ? str : str + TERM_ESC + TERM_RESET;
  }
  _printImage(pixelsArr, output) {
    ...
    for (let row of pixelsArr) {
      ...
      for (let i = 0; i < row.length; i++) {
        ...
        if (
          nextPixel &&
          pixel.every((value, index) => value === nextPixel[index])
        ) {
          block += "  ";
          continue;
        }
        let isTransparent =
          pixel.length >= 3 ? _getIsTransparent(pixel[3]) : false;
